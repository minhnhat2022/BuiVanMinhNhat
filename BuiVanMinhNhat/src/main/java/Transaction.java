...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = BVMN_Blockchain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < BVMN_Blockchain.minimumTransaction) {
            System.out.println("Đầu vào của giao dịch quá bé: " + getInputsValue());
            System.out.println("Vui lòng nhập số lượng giao dịch lớn hơn " + BVMN_Blockchain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionOutput o : outputs) {
            BVMN_Blockchain.UTXOs.put(o.id , o);
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            BVMN_Blockchain.UTXOs.remove(i.UTXO.id);
        }
