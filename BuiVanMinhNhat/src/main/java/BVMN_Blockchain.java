...
public class BVMN_Blockchain {
    public static ArrayList<VNPT_NHAT> blockchain = new ArrayList<VNPT_NHAT>();
    public static int difficulty = 5;
    public static float minimumTransaction = 0.1f;
    public static Store BVMN_Kho; //Kho
    public static Store BVMN_Cuahang; //Cửa hàng
    ...
    public static void main(String[] args) {
        ...
        genesisTransaction = new Transaction(coinbase.publicKey, BVMN_Kho.publicKey, initBalanceA, null);
        ...
        VNPT_NHAT genesis = new VNPT_NHAT("0");
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router trong kho là : " + BVMN_Kho.getBalance());
        System.out.print("Số lượng VNPT-Net Router của Cửa hàng đang còn : ");
        float initBalanceB = sc.nextFloat();
        genesisTransaction = new Transaction(coinbase.publicKey, BVMN_Cuahang.publicKey, initBalanceB, null);
        genesisTransaction.generateSignature(coinbase.privateKey);
        genesisTransaction.transactionId = "0";
        genesisTransaction.outputs.add(new TransactionOutput(genesisTransaction.reciepient, genesisTransaction.value, genesisTransaction.transactionId)); //Thêm Transactions Output
        UTXOs.put(genesisTransaction.outputs.get(0).id, genesisTransaction.outputs.get(0));
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng đang còn là : " + BVMN_Cuahang.getBalance());
        VNPT_NHAT block1 = new VNPT_NHAT(genesis.hash);
        boolean fail = true;
        while (fail){
            ...
            sendFund = BVMN_Kho.sendFunds(BVMN_Cuahang.publicKey, numberTransfer);
            if (sendFund==null){
                continue;
            }else{
                fail= false;
        ...
        System.out.println("Số lượng VNPT-Net Router trong kho hiện tại là : " + BVMN_Kho.getBalance());
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng hiện tại là : " + BVMN_Cuahang.getBalance());
    }
    public static Boolean isChainValid() {
        VNPT_NHAT currentBlock;
        VNPT_NHAT previousBlock;
    ...
    public static void addBlock(VNPT_NHAT newBlock) {
